...
describe("GameFlow integration", () => {
  ...
  it("can start a single player game", async () => {
    ...
    expect(await screen.findByText(/Game type: single/i)).toBeInTheDocument();
  ...
  it("can create a multiplayer game", async () => {
    ...
    expect(await screen.findByText(/Game type: double/i)).toBeInTheDocument();
    expect(screen.getByText(/Join code: ABCD/i)).toBeInTheDocument();
    expect(screen.getByText(/Game status: waiting/i)).toBeInTheDocument();
  });
});
